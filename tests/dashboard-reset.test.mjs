...
test("resets discipline records while preserving master data and personnel", () => {
  const state = {
    ...
    workers: [{
      id: "worker-1", employeeId: 1001, passportNo: 12345678, name: "Worker One", status: "Active",
  ...
  assert.equal(reset.state.workers[0].employeeId, "1001");
  assert.equal(reset.state.workers[0].passportNo, "12345678");
  assert.deepEqual(reset.state.workers[0].offenceLog, original.workers[0].offenceLog);
