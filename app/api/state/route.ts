...
import { migrateLegacyWorkerPolicy } from "../../../lib/disciplinary-policy.mjs";
import {
  mergeState,
  supportedStatePatch,
  validateState,
...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    let snapshot = await readStateSnapshot();
    const migration = migrateLegacyWorkerPolicy(snapshot.state);
    if (migration.changed) {
      const errors = validateStateGroups(migration.state, ["wdp", "workers", "meta"]);
      if (errors.length) throw new Error(errors[0]);
      const result = await writeStateSnapshot({
        expectedRevision: snapshot.revision,
        state: migration.state,
        previousState: snapshot.state,
        changedKeys: ["wdp", "workers", "meta"],
        actor: "system policy migration",
      });
      snapshot = result.ok ? await readStateSnapshot() : result.current;
    }
...
export async function PUT(request: Request): Promise<Response> {
  ...
  try {
    ...
    // Validate only the groups in this update. Historical data-quality gaps in an
    // unrelated group must not block a valid offence, threshold, or personnel save.
    const changedValidationErrors = validateStateGroups(nextState, changedKeys);
    if (changedValidationErrors.length) {
      return json(
        { error: changedValidationErrors[0], details: changedValidationErrors },
        { status: 400 },
      );
    }
