...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (hasSubject && type !== "comment" && type !== "appeal") {
      return json({ error: "Personnel responses must be a comment or an appeal." }, { status: 400 });
    }
    return json({
...
export async function PATCH(request: Request): Promise<Response> {
  ...
  try {
    ...
    const result = await moderateFeedback({ id: body.id, status: body.status, moderatedBy: user.username });
