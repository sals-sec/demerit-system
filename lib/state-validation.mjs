...
function validatePeople(value, key, errors) {
  ...
  value.forEach((person, index) => {
    ...
    // Existing imported records can have a missing employee ID. The interface flags
    // those gaps and requires an ID when an administrator edits the record.
    if (!isOptionalString(person.employeeId, 100)) errors.push(`${path}.employeeId is invalid.`);
...
export function validateStateGroups(state, keys) {
  if (!isObject(state)) return ["A state object is required."];
  const selected = {};
  for (const key of new Set(keys)) {
    if (!STATE_KEY_SET.has(key)) return [`Unsupported state key: ${key}.`];
    if (!(key in state)) return [`Missing state key: ${key}.`];
    selected[key] = state[key];
  }
  return validateState(selected, { partial: true });
}
