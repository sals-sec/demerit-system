...
export async function ensureInitialized(): Promise<void> {
  ...
  const displayName = env.SALS_ADMIN_NAME?.trim() || username;
  if (existing?.credential_version === credentialVersion && existing.role === "super_admin") {
    await database
      .prepare("UPDATE user_accounts SET name = ? WHERE username = ? AND name <> ?")
      .bind(displayName, username, displayName)
      .run();
    return;
  }
...
export async function updateManagedAccount(options: {
  ...
  | { ok: true; account: ManagedAccount; credentialsChanged: boolean }
  | { ok: false; error: string; status: 400 | 403 | 404 | 409 }
> {
  if (options.requestedBy.role !== "super_admin") {
    return { ok: false, error: "Only Super Admin can edit user accounts.", status: 403 };
  }
  const currentUsername = options.currentUsername.trim().toLowerCase();
  const username = options.username.trim().toLowerCase();
  const name = options.name.trim();
  if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(currentUsername)) {
    return { ok: false, error: "A valid current account username is required.", status: 400 };
  }
  if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(username)) {
    return { ok: false, error: "Username must be 3-64 characters using letters, numbers, dots, underscores, or hyphens.", status: 400 };
  }
  if (!name || name.length > 120) {
    return { ok: false, error: "A display name of up to 120 characters is required.", status: 400 };
  }
  const passwordChanged = options.password !== undefined;
  if (passwordChanged && (options.password!.length < 8 || options.password!.length > 1_024)) {
    return { ok: false, error: "Password must be between 8 and 1,024 characters.", status: 400 };
  }

  const database = getD1();
  const account = await database
    .prepare(
      "SELECT username, name, role, credential_version AS credentialVersion, created_at AS createdAt, created_by AS createdBy FROM user_accounts WHERE username = ?",
    )
    .bind(currentUsername)
    .first<ManagedAccount & { credentialVersion: string }>();
  if (!account || (account.role !== "admin" && account.role !== "standard_user")) {
    return { ok: false, error: "The selected managed account no longer exists.", status: 404 };
  }
  ...
  if (usernameChanged) {
    const duplicate = await database
      ...
      .bind(username)
      .first<{ username: string }>();
    if (duplicate) return { ok: false, error: "That username already exists.", status: 409 };
  }
  ...
  if (!usernameChanged && !passwordChanged && name === account.name) {
    return { ok: true, account: managedAccount, credentialsChanged: false };
  }
  ...
  if (passwordChanged) {
    const salt = randomToken();
    const passwordHash = await passwordDigest(options.password!, salt, PASSWORD_ITERATIONS);
    const currentVersion = Number.parseInt(account.credentialVersion, 10);
    const credentialVersion = String((Number.isSafeInteger(currentVersion) ? currentVersion : 0) + 1);
    updateAccount = database.prepare(
      "UPDATE user_accounts SET username = ?, name = ?, password_hash = ?, password_salt = ?, password_algorithm = ?, password_iterations = ?, credential_version = ? WHERE username = ? AND role IN ('admin', 'standard_user')",
    ).bind(
      username,
      name,
      passwordHash,
    ...
    updateAccount = database.prepare(
      "UPDATE user_accounts SET username = ?, name = ? WHERE username = ? AND role IN ('admin', 'standard_user')",
    ).bind(username, name, currentUsername);
  }

  const guardedChange = database.prepare(
    "SELECT CASE WHEN changes() = 1 THEN 1 ELSE json('account update conflict') END AS verified",
  );
  const statements = [updateAccount, guardedChange];
  if (usernameChanged) {
    statements.push(
      database.prepare("UPDATE feedback_submissions SET submitted_by = ? WHERE submitted_by = ?").bind(username, currentUsername),
      database.prepare("UPDATE feedback_submissions SET moderated_by = ? WHERE moderated_by = ?").bind(username, currentUsername),
      database.prepare("UPDATE user_accounts SET created_by = ? WHERE created_by = ?").bind(username, currentUsername),
    );
  }
  if (usernameChanged || passwordChanged) {
    statements.push(database.prepare("DELETE FROM user_sessions WHERE username = ?").bind(currentUsername));
  }
  ...
  try {
    ...
    const stillExists = await database
      ...
      .bind(currentUsername)
      .first<{ username: string }>();
    if (!stillExists) return { ok: false, error: "The selected managed account no longer exists.", status: 404 };
    throw error;
  }
  ...
  return {
    ...
    account: {
      ...
      role: account.role,
      createdAt: account.createdAt,
      createdBy: account.createdBy === currentUsername ? username : account.createdBy,
    },
